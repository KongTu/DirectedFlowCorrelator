...
void plotMixedHarmonics_Qvectors_Psi2HF(){
	...
	for(int eta = 0; eta < 8; eta++){
		for(int charge = 0; charge < 2; charge++){
			...
			threePart[eta][charge] = (TH1D*) file->Get(Form("ana/c2_v1_mixed_HF_%d_%d",eta, charge));
	...
	TFile* file2 = new TFile("mixedHarmonics_tracker_psi2.root");
	TH1D* hist2 = (TH1D*) file2->Get("hist1_0");



	TH1D* base2 = makeHist("base2", "Pb-going", "#eta", "v^{odd}_{1}(#eta)", 100,-3.0,3.0,kBlack);
	base2->GetYaxis()->SetRangeUser(-0.01, 0.01);
	...
	TLegend *w4 = new TLegend(0.23,0.15,0.50,0.3);
    w4->SetLineColor(kWhite);
    w4->SetFillColor(0);
    w4->SetTextSize(24);
    w4->SetTextFont(45);
   	w4->AddEntry(v1_odd[3], "Symmetric sub-events","P");
    w4->AddEntry(hist1[0], "Mixed harmonics 3.0 < |#eta_{#Psi_{2}}| < 4.0","P");
    w4->AddEntry(hist2, "Mixed harmonics 0 < |#eta_{#Psi_{2}}| < 0.8","P");
    w4->Draw("same");
    ...
    c1->Print("mixedHarmonics_zoom.pdf");
