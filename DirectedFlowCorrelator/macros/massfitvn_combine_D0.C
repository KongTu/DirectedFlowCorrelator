...
void massfitvn_combine_D0()
{
    ...
    TFile* file1 = TFile::Open("../rootfiles/D0_CDDF_v1_MB7.root");
...
for(i = 0; i < 6; i++){
    ...
    TH1D* h_mc_match_signal = (TH1D*)file0->Get(Form("mass_pt%d",0));
    TH1D* h_mc_match_all = (TH1D*)file0->Get(Form("mass_all_pt%d",0));
    
    TH1D* h_data = (TH1D*) file1->Get(Form("ana/D0Mass_Hist_%d_2",i));
    ...
    c1[i]->Print(Form("../plots/nominal/mass_%d.pdf",i));
}
