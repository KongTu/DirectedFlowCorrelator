...
void massfitvn_combine_masssingle_ratio()
{
    ...
    //TFile ofile("v2vspt_fromfit_d0_HM185_250_combine_reweightZvtx_shiftpt_v41_etagap2.root","RECREATE");
    ...
    double v2_ncq[13];
    double v2e_ncq[13];
    double ptbin[14] = {-2.0,-1.2,-0.6,0.0,0.6,1.2,2.0};
    double a[13];
    double b[13];
    TGraphErrors* pulldist[13];
    TGraphErrors* datafitratio[13];

    TPad* pad[6][3];
    TLine* Line = new TLine(1.7,1,2,1);
    Line->SetLineStyle(7);
    ...
    for(int i=0;i<6;i++)
    {
        ...
        //TPad* pad[2];
        pad[i][0] = new TPad(Form("pad0_%d",i),Form("pad0_%d",i),0.0,0.46,1,1);
        pad[i][1] = new TPad(Form("pad1_%d",i),Form("pad1_%d",i),0.0,0.25,1,0.46);
        pad[i][2] = new TPad(Form("pad2_%d",i),Form("pad2_%d",i),0.0,0.0,1,0.252);
        ...
        pad[i][1]->SetBottomMargin(0);
        
        pad[i][2]->SetLeftMargin(0.18);
        pad[i][2]->SetRightMargin(0.043);
        pad[i][2]->SetTopMargin(0);
        pad[i][2]->SetBottomMargin(0.165);
    ...
    /*for(int i=0;i<9;i++)
    {
        c[i]->cd()->SetTopMargin(0.06);
        c[i]->cd()->SetLeftMargin(0.18);
        c[i]->cd()->SetRightMargin(0.043);
        c[i]->cd()->SetBottomMargin(0.145);
    ...
    TH1D* hist1 = new TH1D("hist1","",10,1.7,2.0);
    hist1->SetLineWidth(0);
    hist1->SetStats(kFALSE);
    //hist->GetYaxis()->SetRangeUser(0,0.3);
    hist1->GetXaxis()->SetTitle("m_{#piK} (GeV/c^{2})");
    hist1->GetYaxis()->SetTitle("Ratio data/fit");
    ...
    hist1->GetXaxis()->SetTitleSize(0.15);
    hist1->GetYaxis()->SetTitleSize(0.15);
    ...
    hist2->GetXaxis()->SetLabelSize(0.08);
    ...
    for(int i=0;i<6;i++)
    {
        ...
        tex->DrawLatex(0.22,0.86,"30-80%");
        tex->DrawLatex(0.22,0.80,Form("%.1f < y < %.1f",ptbin[i],ptbin[i+1]));
        tex->DrawLatex(0.22,0.74,"p_{T} > 2.0 GeV/c");
        ...
        double xmass[400];
        double pullmass[400];
        double ratiomass[400];
        double ratiomasse[400];
        ...
        for(int k=0;k<h_data->GetNbinsX();k++)
        {
            ...
            ratiomass[k] = h_data->GetBinContent(k)/fmass_combinemassvnfit->Eval(xmass[k]);
            ratiomasse[k] = h_data->GetBinError(k)/fmass_combinemassvnfit->Eval(xmass[k]);
        ...
        pulldist[i] = new TGraphErrors(h_data->GetNbinsX(),xmass,pullmass,0,0);
        pulldist[i]->SetMarkerSize(0.8);
        
        datafitratio[i] = new TGraphErrors(h_data->GetNbinsX(),xmass,ratiomass,0,ratiomasse);
        datafitratio[i]->SetMarkerSize(0.8);
        
        pad[i][1]->cd();
        hist1->Draw();
        datafitratio[i]->Draw("PESAME");
        //pulldist[i]->Draw("PESAME");
        Line->Draw("LSAME");
        ...
        pad[i][0]->cd();
        tex->DrawLatex(0.22,0.62,Form("Chi2/ndf = %.0f/%d",Chi2,ndf));
    ...
    for(int i=0;i<6;i++)
    {
        c[i]->Print(Form("../plots/pull_dist/D0_massfit_pull_rap%d.pdf",i));
    }
    
    // TGraphErrors* v2plot = new TGraphErrors(10,pt,v2,0,v2e);
    // TGraphErrors* v2ncqplot = new TGraphErrors(10,KET_ncq,v2_ncq,0,v2e_ncq);
    // TGraphErrors* v2bkgplot = new TGraphErrors(10,pt,v2_bkg,0,0);
