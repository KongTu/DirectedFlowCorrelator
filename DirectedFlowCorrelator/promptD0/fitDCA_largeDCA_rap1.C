...
void fitDCA_largeDCA_rap1()
{
    double promptCut = 0.008;
    
    TFile* file0 = TFile::Open("bFeedDownPbPbMBMC.hist_rap1.root");
    //TFile* file1 = TFile::Open("Yield_DCA_pPb_pT_EPOS_nonprompt_droplowNbin_reweightZvtx.root");
    
    TFile* file2[8];
    file2[0] = TFile::Open("D0_DCA_data_rap1.root");
    ...
    for(int i=0;i<1;i++)
    {
        ...
        tex->DrawLatex(0.4,0.88,Form("0.6 < |y| < 1.2, 2.0 < p_{T} < 30.0 GeV/c",ptbin[i],ptbin[i+1]));
        ...
        c[i]->Print(Form("plots/largeDCA_promptfit_%d_ratio_rap1.pdf",i));
        
        TFile ofile("prompt_frac_test_1.root","RECREATE");
