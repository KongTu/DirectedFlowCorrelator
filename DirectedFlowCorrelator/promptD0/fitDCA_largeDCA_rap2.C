...
void fitDCA_largeDCA_rap2()
{
    double promptCut = 0.008;
    
    TFile* file0 = TFile::Open("D0_promptNonPrompt_MC_rap2.root");
    //TFile* file1 = TFile::Open("Yield_DCA_pPb_pT_EPOS_nonprompt_droplowNbin_reweightZvtx.root");
    
    TFile* file2[8];
    file2[0] = TFile::Open("D0_DCA_data_rap2.root");
    file2[1] = TFile::Open("D0_DCA_data_rap0.root");
    ...
    double ptbin[10] = {-2.0,-1.2,-0.6,0.0,0.6,1.2,2.0};
    double PromptFrac[3];
    double PromptFracErr[3];
    
    double PromptFrac_cut[3];
    double PromptFracErr_cut[3];
    
    double cutfraction[3];
    TPad* pad[10][2];
    ...
    for(int i=0;i<1;i++)
    {
        ...
        hD0DcaMCPSignal = (TH1D*)file0->Get("hist_prompt_0");
        hD0DcaMCNPSignal = (TH1D*)file0->Get("hist_nonprompt_0");
        ...
        cutfraction[i] = fMix[i]->Integral(0,promptCut)/fMix[i]->Integral(0,fitRangeH);
        
        PromptFrac[i] = promptDYield/(promptDYield+nonpromptDYield);
        PromptFracErr[i] = promptDYieldErr/(promptDYield+nonpromptDYield);
        
        PromptFrac_cut[i] = promptDYield_cut/(promptDYield_cut+nonpromptDYield_cut);
        PromptFracErr_cut[i] = promptDYieldErr_cut/(promptDYield_cut+nonpromptDYield_cut);
        ...
        tex->DrawLatex(0.4,0.88,Form("1.2 < |y| < 2.0, 2.0 < p_{T} < 30.0 GeV/c",ptbin[i],ptbin[i+1]));
        ...
        c[i]->Print(Form("plots/largeDCA_promptfit_%d_ratio_rap2.pdf",i));
        
        TFile ofile("prompt_frac_test.root","RECREATE");
        
        TH1D* hPromptFrac = new TH1D("PromptFrac","PromptFrac",6,ptbin);
        TH1D* hPromptFrac_cut = new TH1D("PromptFrac_cut","PromptFrac_cut",6,ptbin);
